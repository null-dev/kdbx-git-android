...
class WebDavClient(private val config: ServerConfig) {
     ...
     *
     * [p256dh] and [auth] are the Web Push public key and auth secret generated by the UP
     * library. When present the server uses them to encrypt push payloads (RFC 8291).
     * Throws [WebDavException] on a non-2xx response.
     */
    suspend fun registerPushEndpoint(
        ...
        val json = org.json.JSONObject().apply {
            ...
            if (p256dh != null && auth != null) {
                put("keys", org.json.JSONObject().apply {
                    put("p256dh", p256dh)
                    put("auth", auth)
                })
            }
